...
/**
 * Önálló, izolált "hamarosan" (coming soon) widget.
 *
 * Szándékosan NEM függ a közös layero-shop-ui stílustól/szkripttől
 * (get_style_depends / get_script_depends üres), és minden stílusát,
 * viselkedését saját, .lyr-cs alá scope-olt inline blokkból hozza —
 * így a többi widgetre és a shop CSS-re nincs semmilyen hatása.
 */
class Coming_Soon extends Base_Widget {
	public function get_name() {
		return 'layero_coming_soon';
	}

	public function get_title() {
		return __('Layero hamarosan (coming soon)', 'layero-shop-ui');
	}
	...
	protected function register_controls() {
		...
		$this->add_control('kicker', array(
			...
			'default' => __('Készül a webshop', 'layero-shop-ui'),
		));

		$this->add_control('title', array(
			'label' => __('Cím (kiemelés: <span class="hl-amber">…</span>)', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXTAREA,
			'default' => __('Hamarosan új otthona lesz a <span class="hl-amber">személyre szabott ajándékoknak</span>.', 'layero-shop-ui'),
		));

		$this->add_control('lead', array(
			'label' => __('Bevezető (kiemelés: <span class="hl-cyan">…</span>)', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXTAREA,
			'default' => __('Névre szóló világító lámpák, kulcstartók, dekorációk és céges ajándékok — <span class="hl-cyan">egyedi 3D gyártásban</span>, rétegről rétegre. A webshopunkon dolgozunk; iratkozz fel, és elsőként szólunk, amint kinyitunk.', 'layero-shop-ui'),
		));

		$this->add_control('launch', array(
			'label' => __('Indulás dátuma (visszaszámláló)', 'layero-shop-ui'),
			'type' => Controls_Manager::DATE_TIME,
			'default' => '2026-08-15 10:00:00',
			'description' => __('Ha a dátum elmúlt, „Hamarosan!" felirat jelenik meg.', 'layero-shop-ui'),
		));

		$this->add_control('chips', array(
			'label' => __('Kategória-címkék (soronként egy)', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXTAREA,
			'default' => "Tematikus lámpák\nKulcstartók\nDekorációk\nCéges ajándékok",
		));

		$this->add_control('visit_text', array(
			'label' => __('Másodlagos link szöveg', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXT,
			'default' => __('Addig nézd meg a bemutatkozó oldalunkat', 'layero-shop-ui'),
			'separator' => 'before',
		));

		$this->add_control('visit_url', array(
			'label' => __('Másodlagos link URL', 'layero-shop-ui'),
		...
		$this->start_controls_section('notify_section', array('label' => __('Feliratkozás', 'layero-shop-ui')));

		$this->add_control('show_notify', array(
			'label' => __('Feliratkozás mutatása', 'layero-shop-ui'),
			'type' => Controls_Manager::SWITCHER,
			'default' => 'yes',
		));

		$this->add_control('notify_placeholder', array(
			'label' => __('Mező placeholder', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXT,
			'default' => __('Add meg az e-mail-címed', 'layero-shop-ui'),
		));

		$this->add_control('notify_button', array(
			'label' => __('Gomb szöveg', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXT,
			'default' => __('Értesítést kérek', 'layero-shop-ui'),
		));

		$this->add_control('notify_hint', array(
			'label' => __('Apróbetű', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXTAREA,
			'default' => __('Csak az indulásról írunk — spam nélkül, bármikor leiratkozhatsz.', 'layero-shop-ui'),
		));

		$this->add_control('notify_success', array(
			'label' => __('Sikeres üzenet', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXT,
			'default' => __('Köszönjük! Értesítünk, amint elindul a webshop.', 'layero-shop-ui'),
		));

		$this->end_controls_section();

		$this->start_controls_section('contact_section', array('label' => __('Kapcsolat / lábléc', 'layero-shop-ui')));
		...
		$this->add_control('contact_location', array(
			'label' => __('Helyszín', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXT,
			'default' => __('Szatmárnémeti, Románia', 'layero-shop-ui'),
		));

		$this->add_control('copyright', array(
			'label' => __('Copyright név', 'layero-shop-ui'),
			'type' => Controls_Manager::TEXT,
	...
	protected function render() {
		...
		<div class="lyr-cs" data-lyr-cs data-launch="<?php echo esc_attr($launch); ?>">
			<div class="lyr-cs__wrap">
				...
				<?php if (! empty($s['kicker'])) : ?>
					<span class="lyr-cs__kicker"><?php echo esc_html($s['kicker']); ?></span>
				<?php endif; ?>
				...
				<?php if (! empty($s['lead'])) : ?>
					<p class="lyr-cs__lead"><?php echo wp_kses($s['lead'], $title_tags); ?></p>
				<?php endif; ?>

				<div class="lyr-cs__count" role="timer" aria-label="<?php echo esc_attr__('Visszaszámlálás az indulásig', 'layero-shop-ui'); ?>">
					<div class="lyr-cs__cell"><span class="lyr-cs__num" data-d>00</span><span class="lyr-cs__lbl"><?php echo esc_html__('Nap', 'layero-shop-ui'); ?></span></div>
					<div class="lyr-cs__cell"><span class="lyr-cs__num" data-h>00</span><span class="lyr-cs__lbl"><?php echo esc_html__('Óra', 'layero-shop-ui'); ?></span></div>
					<div class="lyr-cs__cell"><span class="lyr-cs__num" data-m>00</span><span class="lyr-cs__lbl"><?php echo esc_html__('Perc', 'layero-shop-ui'); ?></span></div>
					<div class="lyr-cs__cell"><span class="lyr-cs__num" data-s>00</span><span class="lyr-cs__lbl"><?php echo esc_html__('Mp', 'layero-shop-ui'); ?></span></div>
				</div>

				<?php if ($show_notify) : ?>
					<form class="lyr-cs__notify" data-lyr-cs-form novalidate>
						<div class="lyr-cs__row">
							<input type="email" placeholder="<?php echo esc_attr($s['notify_placeholder'] ?? ''); ?>" aria-label="<?php echo esc_attr__('E-mail-cím', 'layero-shop-ui'); ?>" autocomplete="email">
							<button type="submit"><?php echo esc_html($s['notify_button'] ?? __('Értesítést kérek', 'layero-shop-ui')); ?></button>
						</div>
						<?php if (! empty($s['notify_hint'])) : ?>
							<p class="lyr-cs__hint"><?php echo esc_html($s['notify_hint']); ?></p>
						<?php endif; ?>
						<div class="lyr-cs__ok" role="status">
							<svg viewBox="0 0 24 24" fill="none" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="m20 6-11 11-5-5"/></svg>
							<span><?php echo esc_html($s['notify_success'] ?? __('Köszönjük!', 'layero-shop-ui')); ?></span>
						</div>
					</form>
				<?php endif; ?>

				<?php if (! empty($chips)) : ?>
					<div class="lyr-cs__chips" aria-hidden="true">
				...
				<?php if (! empty($s['visit_text']) && ! empty($visit_url)) : ?>
					<a class="lyr-cs__visit" href="<?php echo esc_url($visit_url); ?>"<?php echo $visit_target . $visit_rel; ?>>
			...
			<footer class="lyr-cs__foot">
				<div class="lyr-cs__foot-in">
					<?php if (! empty($email)) : ?>
					...
					<?php if (! empty($s['contact_location'])) : ?>
						<span class="lyr-cs__loc"><svg viewBox="0 0 24 24" fill="none" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg><?php echo esc_html($s['contact_location']); ?></span>
					<?php endif; ?>
					<p class="lyr-cs__copy">© <?php echo esc_html($year); ?> <?php echo esc_html($s['copyright'] ?? 'Layero 3D Design'); ?></p>
				</div>
			</footer>
		</div>
		...
		<script>
		(function () {
			var roots = document.querySelectorAll('[data-lyr-cs]');
			for (var i = 0; i < roots.length; i++) {
				(function (root) {
					if (root.getAttribute('data-lyr-init')) { return; }
					root.setAttribute('data-lyr-init', '1');

					var target = new Date(String(root.getAttribute('data-launch') || '').replace(' ', 'T')).getTime();
					var box = root.querySelector('.lyr-cs__count');
					...
					function tick() {
						...
						if (diff <= 0) {
							if (box) { box.innerHTML = '<div class="lyr-cs__cell" style="min-width:auto;padding-inline:26px"><span class="lyr-cs__num" style="font-size:clamp(1.3rem,4vw,2rem)">Hamarosan!</span></div>'; }
							clearInterval(t);
							return;
						}
					...
					var form = root.querySelector('[data-lyr-cs-form]');
					if (form) {
						...
						form.addEventListener('submit', function (e) {
							...
							if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)) {
								...
								var row = form.querySelector('.lyr-cs__row');
